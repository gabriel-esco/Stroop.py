...
if __name__ == '__main__':
    ...
    #set distraction gamemode
    game.gamemode = 1
    
    #run game until reaching 20 trials and store score 
    while True:
        game.play_step()
        if game.gameCounter == 20:
            break
    distracted_score = copy.deepcopy(game.score)

    #set non-distraction gamemode, reset score and counter
    game.gamemode = 0
    game.gameCounter = 0
    game.score = 0
    while True:
    ...
    with open('stroopV2_data.csv', mode='w') as stroop_file:
        stats = ['game_mode', 'trial', 'result', 'response_time']
        writer = csv.writer(stroop_file)
        for row in game.stats:
            writer.writerow(row)
    
    print('Final Distracted Score: ' + str(distracted_score) + "/20")
    print('Final Non-Distracted Score: ' + str(score) +"/20")
